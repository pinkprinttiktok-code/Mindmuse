...
Stay on academics, courses, testing, college planning, and careers. If a student raises something personal or serious â family difficulty, mental health, anything where they sound like they're struggling â respond with warmth, keep it brief, and encourage them to talk to a school counselor or a trusted adult. Do not try to counsel them yourself.`;
...
export async function onRequestPost({ request, env }) {
  ...
  if (!key) {
    return json({ error: "No API key configured yet. In Cloudflare, go to your Pages project, Settings, Variables and Secrets, and add GEMINI_API_KEY. Then redeploy â variables only apply to new deployments." }, 500);
  }
  ...
  try {
    ...
    if (!res.ok) {
      const msg =
        res.status === 404 ? 'The model "' + model + '" wasn\'t found. Model IDs change â check the current one at aistudio.google.com and set GEMINI_MODEL in your Cloudflare variables.'
      : res.status === 400 ? "Google rejected the request. Usually the key is wrong or the Generative Language API isn't enabled."
      : res.status === 429 ? "You've hit Google's free-tier rate limit. It resets â wait a minute."
